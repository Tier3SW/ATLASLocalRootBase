...
# do we have an error here ?
    ...
    if [[ $alrb_swVersionN -le $alrb_currentJavaVersionN ]] && [[ $alrb_Force != "YES" ]]; then
	alrb_java_skip="Java $alrb_currentJavaVersion already available"
	return 0
    else
	alrb_java_skip=""
    fi
	...
	local alrb_tmpAr=( `\find  /Library/Java/JavaVirtualMachines -maxdepth 1 -mindepth 1 -type d` )
    else
	local alrb_tmpAr=()
    fi
	...
	if [ ! -e "$alrb_item/Contents/Home/jre/bin/java" ]; then
	    continue
	fi
	local alrb_itemVer=`\echo $alrb_item | \sed -e 's/^.*jdk\(.*\).jdk/\1/g'`
	...
	if [ $? -ne 0 ]; then
	    ...
	    alrb_java_extra="$alrb_java_extra,alrb_javaHome=${alrb_cand}/Contents/Home/jre"
	fi
	return 0
    else
