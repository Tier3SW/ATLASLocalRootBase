...
# eg root built with xrootd3 has issues with xrootd4 so warn major changes
    if [[ "$alrb_XrootdRootFromAthena" != "YES" ]] && [[ ! -z $ROOTSYS ]] && [[ ! -z $alrb_rootXrdMinVer ]]; then
	let alrb_xrdVerMin=`\echo $alrb_rootXrdMinVer | \cut -f 1 -d "."`
	let alrb_xrdVerCur=`\echo $ATLAS_LOCAL_XROOTD_VERSION | \cut -f 1 -d "."`
	if [[ $alrb_xrdVerMin -eq 3 ]] && [[ $alrb_xrdVerMin -ne $alrb_xrdVerCur ]]; then
	    \echo " xrootd:"
