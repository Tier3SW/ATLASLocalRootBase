...
Usage: lsetup [global options] "${alrb_sw} [options] <version>"
    ...
    This sets up the ATLAS environment for tbb
...
alrb_fn_tbbVersionConvert()
{
    \echo $1 | \cut -f 1 -d "-" | sed -e 's/_//g'
    return $?
}
...
# do we have an error here ?
    ...
    if [ ! -z $ALRB_tbbVersion ]; then
	local alrb_swVersion=$ALRB_tbbVersion
    else
...
# dependencies
	...
	\echo "Error: tbb version undetermined or unavailable $alrb_swVersion" 1>&2
	return 64
    fi
}
