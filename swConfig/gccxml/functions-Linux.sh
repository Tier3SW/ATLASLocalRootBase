...
Usage: lsetup [global options] "${alrb_sw} [options] <version>"
    ...
    This sets up the ATLAS environment for gccxml
...
alrb_fn_gccxmlDepend()
{
    local alrb_sw="gccxml"
...
# do we have an error here ?
    ...
    if [ ! -z $ALRB_gccxmlVersion ]; then
	local alrb_swVersion=$ALRB_gccxmlVersion
    else
