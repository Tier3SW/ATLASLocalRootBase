...
Usage: lsetup [global options] "${alrb_sw} [options] <version>"
    ...
    This sets up the ATLAS environment for gitlab
...
alrb_fn_gitlabDepend()
{
    local alrb_sw="gitlab"
...
# do we have an error here ?
    ...
    if [ ! -z $ALRB_gitlabVersion ]; then
	local alrb_swVersion=$ALRB_gitlabVersion
    else
...
# python
	...
	\echo "Error: gitlab version undetermined or unavailable $alrb_swVersion" 1>&2
	return 64
    fi
}
