...
Usage: lsetup [global options] "${alrb_sw} [options] <version>"
    ...
    This sets up the ATLAS environment for png
...
# do we have an error here ?
    ...
    if [ ! -z $ALRB_pngVersion ]; then
	local alrb_swVersion=$ALRB_pngVersion
    else
...
# dependencies
	...
	\echo "Error: png version undetermined or unavailable $alrb_swVersion" 1>&2
	return 64
    fi
}
