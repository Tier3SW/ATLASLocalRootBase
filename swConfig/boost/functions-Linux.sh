...
Usage: lsetup [global options] "${alrb_sw} [options] <version>"
    ...
    This sets up the ATLAS environment for boost
...
# do we have an error here ?
    ...
    if [ ! -z $ALRB_boostVersion ]; then
	local alrb_swVersion=$ALRB_boostVersion
    else
...
# dependencies
	...
	\echo "Error: boost version undetermined or unavailable $alrb_swVersion" 1>&2
	return 64
    fi
}
