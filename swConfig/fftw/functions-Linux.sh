...
Usage: lsetup [global options] "${alrb_sw} [options] <version>"
    ...
    This sets up the ATLAS environment for fftw
...
# do we have an error here ?
    ...
    if [ ! -z $ALRB_fftwVersion ]; then
	local alrb_swVersion=$ALRB_fftwVersion
    else
...
# dependencies
	...
	\echo "Error: fftw version undetermined or unavailable $alrb_swVersion" 1>&2
	return 64
    fi
}
