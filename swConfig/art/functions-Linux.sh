...
Usage: lsetup [global options] "${alrb_sw} [options] <version>"
    ...
    This sets up the ATLAS environment for art
...
# do we have an error here ?
    ...
    if [ ! -z $ALRB_artVersion ]; then
	local alrb_swVersion=$ALRB_artVersion
    else
...
# dependencies
	...
	\echo "Error: art version undetermined or unavailable $alrb_swVersion" 1>&2
	return 64
    fi
}
