...
Usage: lsetup [global options] "${alrb_sw} [options] <version>"
    ...
    This sets up the ATLAS environment for swmgr
...
# do we have an error here ?
    ...
    if [ ! -z $ALRB_swmgrVersion ]; then
	local alrb_swVersion=$ALRB_swmgrVersion
    else
...
# dependencies
	...
	\echo "Error: swmgr version undetermined or unavailable $alrb_swVersion" 1>&2
	return 64
    fi
}


alrb_fn_swmgrGetInstallDirAttributes()
{
    \echo "sw-mgr"
