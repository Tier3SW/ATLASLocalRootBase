...
Usage: lsetup [global options] "${alrb_sw} [options] <version>"
    ...
    This sets up the ATLAS environment for Pacman
...
alrb_fn_pacmanVersionConvert()
{
# 2 significant figures 
    local alrb_tmpVal=`\echo $1 | \cut -f 2 -d "-"`
    $ATLAS_LOCAL_ROOT_BASE/utilities/convertToDecimal.sh $alrb_tmpVal 2
    return $?
}


alrb_fn_pacmanDepend()
{
    local alrb_sw="pacman"
...
# do we have an error here ?
    ...
    if [ ! -z $ALRB_pacmanVersion ]; then
	local alrb_swVersion=$ALRB_pacmanVersion
    else
...
# dependencies
	...
	\echo "Error: Pacman version undetermined or unavailable $alrb_swVersion" 1>&2
	return 64
    fi
}


alrb_fn_pacmanPostInstall()
{
    cd $alrb_InstallDir
    source ./setup.sh
    csh -tc "source ./setup.csh"
